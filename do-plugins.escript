...
main([]) ->
    ...
    io:format("Deps dirs to search for plugins: ~p~n",[DepDirs]),
    ...
    case {Includes, Statics, Templates} of
        {[],[],[]} -> 
            ...
            io:format("Generating blank plugin header (plugins.hrl)~n"),
...
generate_plugin_include(Config, Includes) ->
    IncludeFile = proplists:get_value(plugins_hrl, Config, "include/plugins.hrl"),
